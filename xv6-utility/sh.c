...
checkcmd(char *buf)
{
  ...
  if(buf[0] == 'b' && buf[1] == 'l' && buf[2] == 'o' && buf[3] == 'c' && buf[4] == 'k' && buf[5] == ' '){
    const char *call = (buf + 6);
    int syscall_id = atoi(call);
    if(block(syscall_id) < 0){
      printf(2, "cannot block syscall %d\n", syscall_id);
      return 1;
    }
    return 0;
  }
  if(buf[0] == 'u' && buf[1] == 'n' && buf[2] == 'b' && buf[3] == 'l' && buf[4] == 'o' && buf[5] == 'c' && buf[6] == 'k' && buf[7] == ' '){
    const char *call = (buf + 8);
    int syscall_id = atoi(call);
    if(unblock(syscall_id) < 0){
      printf(2, "cannot unblock syscall %d\n", syscall_id);
      return 1;
    }
    return 0;
  }
  if(buf[0] == 'c' && buf[1] == 'h' && buf[2] == 'm' && buf[3] == 'o' && buf[4] == 'd' && buf[5] == ' '){
    ...
    *args = '\0';args++;
    ...
    if (chmod(filename, mode) < 0) {
      printf(2, "chmod: could not change file permissions\n");
      return 1;
    }
    return 0;
  }
